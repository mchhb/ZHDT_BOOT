...
#include "LOOP.h"
#include "24c16.h"
uint8 buff[256];				                             /*定义一个2KB的缓存*/
extern uint8_t work_Acc_success;
uint8  BD_TG_server_ip[4];//{202,106,5,12};        //BD_TG服务器IP地址
uint16 BD_TG_server_port;                   //BD_TG服务器端口号
...
const char *topics1[] = {"/hardware-upgrade"}; //原来的主题名称stateUp
const char *topics2[] = {"/hardware-upgrade-result"}; //原来的主题名称stateUp
...
//					printf("进入发布消息\r\n");
				...
				if(send_version == 1)
				{
					...
					delay_ms(100);
					BD_TG_Publish(*topics2,test,strlen(test)); //发布消息	
				}
				else
				{
