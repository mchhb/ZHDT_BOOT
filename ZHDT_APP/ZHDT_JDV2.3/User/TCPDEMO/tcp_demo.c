...
#include "24c16.h"
#include "input.h"
uint8 buff[256];				                             /*¶¨ÒåÒ»¸ö2KBµÄ»º´æ*/
uint8  BD_TG_server_ip[4];//{202,106,5,12};        //BD_TG·þÎñÆ÷IPµØÖ·
uint16 BD_TG_server_port;                   //BD_TG·þÎñÆ÷¶Ë¿ÚºÅ
...
const char *topics1[] = {"/hardware-upgrade"}; //Ô­À´µÄÖ÷ÌâÃû³ÆstateUp
const char *topics3[] = {"/hardware-upgrade-result"}; //Ô­À´µÄÖ÷ÌâÃû³ÆstateUp
			...
			switch(MQTT_STATE)
			{
			 ...
			 if(((door_open_times == 1)&&(door_over_time_alway == 0))||(door_printf_mqtt == 1))
				...
				if(send_version == 1)
				{
					...
					delay_ms(100);
					BD_TG_Publish(*topics3,test,strlen(test)); //·¢²¼ÏûÏ¢	
				}
