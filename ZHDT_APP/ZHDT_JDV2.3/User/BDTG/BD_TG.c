...
#define PROID		"gdzy123"							// Éè±¸È«³Æ//Ö®Ç°ÓÃ»§ÃûÊÇ£º96ppxbd/s001
//#define PROID		"admin"
#define AUTH_INFO	"hit@gdzy"	// APIÃÜÔ¿  //Ö®Ç°ÃÜÂëÊÇc8opPYlA7ejO7RxAmMjynNBBf015UaGf04UxAIowQIU=
...
extern uint8_t w5500_success_crc;
...
void BD_TG_RevPro(unsigned char *cmd)
{
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//½ÓÊÕµÄPublishÏûÏ¢
			...
			{
				sub_flag = 1;
				sub_rev_length = strlen(req_payload);
			//	printf("topic: %s\r\npayload: %s\r\n", cmdid_topic, req_payload);
				memcpy(publish_buf,req_payload,strlen(req_payload));
//				printf("%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x\r\n",publish_buf[0],publish_buf[1],publish_buf[2],publish_buf[3],publish_buf[4],publish_buf[5],publish_buf[6],publish_buf[7],publish_buf[8],publish_buf[9],publish_buf[10],publish_buf[11],publish_buf[12]);
				switch(qos)
				{
