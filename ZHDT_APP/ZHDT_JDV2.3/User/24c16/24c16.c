...
{
  GPIO_SetBits(GPIOD, I2C_SDA);
  GPIO_SetBits(GPIOB, I2C_SCK);
  delay_us(1);
  GPIO_ResetBits(GPIOD, I2C_SDA);
...
unsigned char read_1byte(void)
{
  ...
  GPIO_ResetBits(GPIOB, I2C_SCK);delay_us(1);GPIO_SetBits(GPIOD, I2C_SDA);
  for (i=0;i<8;i++)
  {
    delay_us(1);
    GPIO_SetBits(GPIOB, I2C_SCK);
    ...
    if(GPIO_ReadInputDataBit(GPIOD, I2C_SDA)==1) j=1; //???SDA becoms inout pin from output pin;
    else j=0;
    k=(k<<1) | j; 
    GPIO_ResetBits(GPIOB, I2C_SCK);
  }
...
void clock(void)
{
  ...
  while((GPIO_ReadInputDataBit(GPIOD, I2C_SDA)==1) && (i<255))
    i++;
  GPIO_ResetBits(GPIOB, I2C_SCK);
  delay_us(1);
}
